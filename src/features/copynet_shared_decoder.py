...
from allennlp.data.fields import TextField, ArrayField, MetadataField,\
    NamespaceSwappingField, LabelField
...
@DatasetReader.register("copynet_shared_decoder")
class CopyNetSharedDecoderDatasetReader(CopyNetDatasetReader):
    """
    ...
    and produces a dataset suitable for the ``CopyNetSharedDecoder`` model
    or any model with a matching API.

    An instance produced by ``CopyNetSharedDecoderDatasetReader`` will contain at least the following fields:
